...
class RandomPerspective:
    ...
    def affine_transform(self, img, border,mask):
        """Center."""
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                mask = cv2.warpPerspective(mask, M, dsize=self.size, borderValue=(0,0,0))
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                mask = cv2.warpAffine(mask, M[:2], dsize=self.size, borderValue=(0,0,0))
        return img, M, s,mask
    ...
    def __call__(self, labels):
        ...
        img, M, scale,mask = self.affine_transform(img, border,mask)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None, mask=None):
        """Return updated labels and image with added border."""
        if labels is None:
            labels = {}
        img = labels.get('img') if image is None else image
        mask = labels.get('mask') if mask is None else mask
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            if mask is not None and not mask.size.empyt():
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            ins_flip.fliplr(w)  # 存在问题 mask 可能需要对应操作
            ...
            # mask = cv2.filp(mask, 1).astype(bool)
            im[i] = result[i]
...
class Albumentations:
    """
    相册转换。可选，卸载要禁用的程序包。”。
    应用模糊、中值模糊、转换为灰度、对比度受限自适应直方图均衡、，
    亮度和对比度的随机变化、RandomGamma和通过压缩降低图像质量
